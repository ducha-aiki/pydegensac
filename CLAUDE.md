...
1. **Legacy C core** — `src/pydegensac/degensac/` (RANSAC/DEGENSAC algorithms, e.g. `ranH.c`, `ranF.c`, `exp_ranH.c`, `exp_ranF.c`) and `src/pydegensac/matutls/` (linear algebra utilities). This is decades-old procedural C linked against LAPACK; preserve its naming and style when editing. Compiled into static libs `pydegensac_support` and `matutls` by the top-level `CMakeLists.txt`.
